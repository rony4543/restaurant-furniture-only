...
## Frontend Aesthetic & Writing Principles

1. **Distinctive Point of View:** Make deliberate, opinionated choices about palette, typography, and layout that are specific to the brief. Avoid templated, generic designs (like numbered markers just for decoration).
2. **Typography as Personality:** Set a clear type scale with intentional weights, widths, and spacing. Use a characterful display face with restraint and a complementary body face.
3. **Restraint:** Spend your boldness in one place. Let a single signature element be memorable, and keep everything around it quiet and disciplined. Cut any decoration that does not serve the brief.
4. **Copy as Design Material:** Write from the end user's side of the screen using active voice. Explain what went wrong in errors without apologizing. An empty screen is an invitation to act. Keep it conversational and tuned.

## Component & Library References

You have full source access to several modern UI libraries in `references/libs/`. Before implementing a component from scratch, search these directories to use or adapt their high-quality, animated, and accessible implementations:

- **Inspira UI:** `references/libs/inspira-ui/`
- **Animate UI:** `references/libs/animate-ui/`
- **Lenis (Smooth Scrolling):** `references/libs/lenis/`

When asked to "review my UI", "audit design", or "check my site against best practices", always ensure compliance with the principles in this document and prioritize structural integrity, accessibility, and distinctive design over simple aesthetic tweaks.
